...
class FarmaciaService extends ChangeNotifier {
  final String _baseUrl = "productos-6194b-default-rtdb.firebaseio.com";


  final List<Farmacias> productos = [];

  bool isLoading = true;
  bool isSaving = false;

  Farmacias? productoSeleccionado;
  ...
  Future obtenerProductos() async {
    this.isLoading = true;
    notifyListeners();
    final url = Uri.https(_baseUrl, 'farmacia.json');
    ...
    productosMap.forEach((key, value) {
      final productoTemp = Farmacias.fromMap(value);
      productoTemp.id = key;
  ...
  Future<String> updateProducto(Farmacias producto) async {
    final url = Uri.https(_baseUrl, 'farmacia/${producto.id}.json');
  ...
  Future saveOrCreateProducto(Farmacias producto) async {
    isSaving = true;
    notifyListeners();
  ...
  Future<String> createProducto(Farmacias producto) async {
    final url = Uri.https(_baseUrl, 'farmacia.json');
