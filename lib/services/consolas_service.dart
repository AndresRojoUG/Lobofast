...
class ConsolaService extends ChangeNotifier {
  final String _baseUrl = "productos-6194b-default-rtdb.firebaseio.com";


  final List<Consolas> productos = [];

  bool isLoading = true;
  bool isSaving = false;

  Consolas? productoSeleccionado;
  ...
  Future obtenerProductos() async {
    this.isLoading = true;
    notifyListeners();
    final url = Uri.https(_baseUrl, 'consolas.json');
    ...
    productosMap.forEach((key, value) {
      final productoTemp = Consolas.fromMap(value);
      productoTemp.id = key;
  ...
  Future<String> updateProducto(Consolas producto) async {
    final url = Uri.https(_baseUrl, 'consolas/${producto.id}.json');
  ...
  Future saveOrCreateProducto(Consolas producto) async {
    isSaving = true;
    notifyListeners();
  ...
  Future<String> createProducto(Consolas producto) async {
    final url = Uri.https(_baseUrl, 'consolas.json');
